...
const navItems = [
  ...
  },
  {
    ...
    label: "Folders",
    icon: <svg className="h-4 w-4 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M2.25 12.75V12A2.25 2.25 0 0 1 4.5 9.75h15A2.25 2.25 0 0 1 21.75 12v.75m-8.69-6.44-2.12-2.12a1.5 1.5 0 0 0-1.061-.44H4.5A2.25 2.25 0 0 0 2.25 6v8.25" /></svg>,
  },
  {
...
export default function Sidebar({ isOpen, onClose, isSuperAdmin = false }: SidebarProps) {
    ...
    <>
      ...
      >
        ...
        <nav className={`flex-1 space-y-0.5 py-4 ${collapsed ? "lg:px-1.5 px-3" : "px-3"}`}>
          ...
          {navItems.filter(item => !("adminOnly" in item && item.adminOnly && !isSuperAdmin)).map((item) => {
