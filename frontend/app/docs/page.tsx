...
function Code({ children }: { children: string }) {
  return (
    <code className="rounded bg-gg-s2 px-1.5 py-0.5 text-xs text-violet-400 font-mono">
      {children}
    </code>
  );
}
...
function Block({ label, children }: { label?: string; children: string }) {
  return (
    <div className="rounded-xl border border-gg-border bg-black/20 overflow-hidden">
      {label && (
        <div className="border-b border-gg-border px-4 py-2 text-[10px] font-semibold uppercase tracking-wider text-gg-text3">
          {label}
        </div>
      )}
...
function Note({ type, children }: { type: "info" | "warn" | "tip"; children: React.ReactNode }) {
  ...
  return (
    <div className={`rounded-xl border ${styles.border} ${styles.bg} p-4`}>
      ...
      <span className="text-xs text-gg-text2 leading-relaxed">{children}</span>
    </div>
  );
}
...
function NumberedList({ items }: { items: React.ReactNode[] }) {
  return (
    <ol className="space-y-2.5">
      {items.map((item, i) => (
        <li key={i} className="flex gap-3 text-sm text-gg-text2">
          <span className="mt-0.5 flex h-5 w-5 flex-shrink-0 items-center justify-center rounded-full bg-gg-s2 text-[10px] font-bold text-gg-text3">
            {i + 1}
          </span>
...
export default function DocsPage() {
  ...
  return (
    <div className="min-h-screen bg-gg-bg text-gg-text">
      {/* ── Navbar ─────────────────────────────────────────── */}
      <nav className="sticky top-0 z-50 border-b border-gg-border bg-gg-bg/80 backdrop-blur-xl">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-3 sm:px-6">
          <Link href="/" className="flex items-center gap-2">
            ...
            <span className="text-sm font-semibold">GDriveGenie</span>
            <span className="rounded-full border border-gg-border px-2 py-0.5 text-[10px] text-gg-text3">Docs</span>
          </Link>
      ...
      <div className="mx-auto max-w-7xl px-4 py-12 sm:px-6">
        ...
        <div className="mb-16 max-w-2xl">
          <div className="mb-4 inline-flex items-center gap-2 rounded-full border border-violet-500/20 bg-violet-600/5 px-3 py-1">
            <span className="h-1.5 w-1.5 rounded-full bg-violet-500" />
            <span className="text-xs font-medium text-violet-400">Setup Guide</span>
          </div>
          <h1 className="mb-4 text-4xl font-bold tracking-tight text-gg-text md:text-5xl">
            Get up and running
            <br />
            <span className="gradient-text">in 15 minutes</span>
          </h1>
          <p className="text-lg leading-relaxed text-gg-text2">
          ...
          <div className="mt-6 flex flex-wrap gap-2">
            {["Python 3.10+", "Node.js 18+", "Google Account(s)", "Git"].map((p) => (
              <span key={p} className="rounded-lg border border-gg-border bg-gg-s1 px-3 py-1.5 text-xs text-gg-text2">
                {p}
              </span>
            ))}
          </div>
        ...
        <div className="flex gap-12">
          ...
          <aside className="hidden w-48 flex-shrink-0 lg:block">
            <div className="sticky top-20">
              <p className="mb-3 text-[10px] font-semibold uppercase tracking-wider text-gg-text3">Steps</p>
              <nav className="space-y-1">
                {steps.map((s) => (
                  <a
                    key={s.id}
                    href={`#${s.id}`}
                    className="flex items-center gap-2.5 rounded-lg px-2 py-1.5 text-xs text-gg-text2 transition hover:bg-gg-hover hover:text-gg-text"
                  >
                    <span className={`text-[10px] font-bold tabular-nums ${s.color}`}>{s.num}</span>
                    <span>{s.title}</span>
                  </a>
                ))}
                <div className="my-3 border-t border-gg-border" />
                <a href="#faq" className="flex items-center gap-2.5 rounded-lg px-2 py-1.5 text-xs text-gg-text2 transition hover:bg-gg-hover hover:text-gg-text">
                  <span className="text-[10px] font-bold tabular-nums text-gg-text3">—</span>
                  <span>FAQ</span>
                </a>
                <a href="#troubleshooting" className="flex items-center gap-2.5 rounded-lg px-2 py-1.5 text-xs text-gg-text2 transition hover:bg-gg-hover hover:text-gg-text">
                  <span className="text-[10px] font-bold tabular-nums text-gg-text3">—</span>
          ...
          <div className="min-w-0 flex-1 space-y-16">
            ...
            {/* ── Step 01 ── */}
            <section id="clone">
              <StepHeader num="01" color="text-violet-400" accent="border-violet-500/40" title="Clone the Repository" />
              <div className="space-y-4">
                <p className="text-sm leading-relaxed text-gg-text2">
...
cd GDriveGenie`}
            ...
            <section id="credentials">
              ...
              <div className="space-y-5">
                <p className="text-sm leading-relaxed text-gg-text2">
                  You only need <strong className="text-gg-text">one Google Cloud project</strong> and one credentials file — no matter how many Drive accounts you want to add. All accounts authenticate through the same OAuth client.
                </p>
                <NumberedList items={[
                  <>Go to <a href="https://console.cloud.google.com" target="_blank" rel="noopener noreferrer" className="text-violet-400 underline underline-offset-2">console.cloud.google.com</a> and create a <strong className="text-gg-text">new project</strong>.</>,
                  <>Navigate to <strong className="text-gg-text">APIs &amp; Services → Library</strong>, search for <strong className="text-gg-text">Google Drive API</strong>, and enable it.</>,
                  <>Go to <strong className="text-gg-text">APIs &amp; Services → OAuth consent screen</strong>. Choose <strong className="text-gg-text">External</strong>, fill in any app name, and add <strong className="text-gg-text">all Google accounts you want to connect</strong> as test users (one email per line).</>,
                  <>Go to <strong className="text-gg-text">Credentials → Create Credentials → OAuth client ID</strong>. Choose <strong className="text-gg-text">Web application</strong> as the application type. Under <strong className="text-gg-text">Authorized redirect URIs</strong>, add <Code>http://localhost:8000/api/auth/callback</Code>.</>,
...
└── credentials.json   ← your single OAuth client file`}
                ...
                <Note type="warn">
                  The <Code>config/</Code> folder is intentionally excluded from Git — its contents are listed in <Code>.gitignore</Code> so your credentials are <strong className="text-gg-text">never</strong> accidentally committed. The folder itself is tracked as an empty placeholder (via <Code>.gitkeep</Code>), so it already exists after cloning. Just drop your downloaded JSON file inside it.
                </Note>
                <Note type="info">
...
{`python backend/scripts/generate_secrets.py`}
                ...
                <div className="rounded-xl border border-gg-border bg-gg-s1 p-5">
                  <p className="mb-3 text-xs font-semibold uppercase tracking-wider text-gg-text3">What the script does</p>
                  <ul className="space-y-2">
                    {[
                      ...
                      <li key={k} className="flex gap-3 text-xs">
                        <span className="flex-shrink-0 font-semibold text-gg-text">{k}</span>
                        <span className="text-gg-text2">{v}</span>
                      </li>
                    ))}
                  </ul>
                </div>
                <Note type="info">
                  All three secrets are stored in the <Code>app_config</Code> table inside <Code>backend/gdriveGenie.db</Code> (or the <Code>gdriveGenie_data</Code> Docker volume when running with Docker Compose). Re-run the script at any time to change your PIN — it will ask before overwriting.
                </Note>
              </div>
            </section>
...
{`uvicorn backend.main:app --reload --port 8000`}
                ...
                <Note type="tip">
                  Verify it works by opening <span className="font-mono text-violet-400">http://localhost:8000/docs</span> — you should see the Swagger UI with all API routes.
                </Note>
              </div>
            </section>
...
npm run dev`}
                ...
                <p className="text-sm text-gg-text2">
                  Open <span className="font-mono text-violet-400">http://localhost:3000</span>. The Next.js config rewrites all <Code>/api/*</Code> requests to <Code>http://localhost:8000/api/*</Code> automatically.
                </p>
              </div>
            </section>
            ...
            <section id="connect">
              ...
              <div className="space-y-5">
                ...
                <NumberedList items={[
                  <>Navigate to <span className="font-mono text-violet-400">http://localhost:3000/login</span> and enter your PIN.</>,
                  <>Click <strong className="text-gg-text">Settings</strong> in the left sidebar.</>,
                  <>Click the <strong className="text-gg-text">Connect another account</strong> button in the top-right of the Settings page.</>,
                  <>Google will show the <strong className="text-gg-text">account chooser</strong> — select the Google account you want to add, then approve the Drive permission on the consent screen.</>,
            ...
            <section id="scale">
              ...
              <div className="space-y-4">
                ...
                <NumberedList items={[
                  <>Go to <strong className="text-gg-text">Settings</strong> and click <strong className="text-gg-text">Connect another account</strong>.</>,
                ...
                <div className="rounded-xl border border-emerald-500/20 bg-emerald-500/5 p-5 text-center">
                  <p className="text-2xl font-bold text-gg-text">N × 15 GB</p>
                  <p className="mt-1 text-xs text-gg-text2">No hard limit on accounts. 10 accounts = 150 GB free.</p>
                </div>
              </div>
            </section>
            ...
            <section>
              <div className="mb-8">
                <p className="mb-1 text-xs font-semibold uppercase tracking-wider text-gg-text3">Feature Reference</p>
                <h2 className="text-2xl font-bold text-gg-text">What&apos;s inside the dashboard</h2>
              </div>
              <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                {[
                  {
                    label: "Overview", color: "text-violet-400", bg: "bg-violet-600/10", border: "border-violet-500/20",
                  ...
                  <div key={page.label} className={`rounded-2xl border ${page.border} bg-gg-s1 p-5`}>
                    <div className={`mb-3 inline-flex rounded-lg px-2.5 py-1 text-xs font-semibold ${page.color} ${page.bg}`}>{page.label}</div>
                    <ul className="space-y-1.5">
                      {page.items.map((item) => (
                        <li key={item} className={`flex items-start gap-2 text-xs text-gg-text2`}>
            ...
            <section id="faq">
              <div className="mb-8">
                <p className="mb-1 text-xs font-semibold uppercase tracking-wider text-gg-text3">FAQ</p>
                <h2 className="text-2xl font-bold text-gg-text">Frequently Asked Questions</h2>
              </div>
              <div className="divide-y divide-gg-border rounded-2xl border border-gg-border bg-gg-s1 overflow-hidden">
                {[
                  {
                    q: "Is my data safe?",
                    a: "Yes. GDriveGenie runs entirely on your local machine. Files are stored in your own Google Drive accounts. OAuth tokens are encrypted with AES-128 (Fernet) before being written to the local database.",
                  },
                  {
                  ...
                  },
                  {
                    ...
                    a: "GDriveGenie always routes to the account with the most free space. If all accounts are full, the upload fails with a 503 error. Add another account to fix it.",
                  },
                  {
                    q: "Can I access files I added directly in Google Drive?",
                    a: "Yes. Click Sync in the Files page. GDriveGenie syncs only metadata — not file content — so it's fast.",
                  },
                  {
            ...
            <section id="troubleshooting">
              <div className="mb-8">
                <p className="mb-1 text-xs font-semibold uppercase tracking-wider text-gg-text3">Troubleshooting</p>
                <h2 className="text-2xl font-bold text-gg-text">Common Issues</h2>
              </div>
              <div className="space-y-3">
                {[
                  ...
                  },
                  {
                    ...
                    a: "GDriveGenie doesn't watch Drive in real time. Click the Sync button in the Files page to pull in new files.",
                  },
                  {
                  ...
                  <div key={item.q} className="rounded-xl border border-gg-border bg-gg-s1 p-5">
                    <p className="mb-1.5 flex items-start gap-2 text-sm font-semibold text-gg-text">
                      <span className="mt-0.5 flex-shrink-0 text-amber-400">!</span>
                      {item.q}
                    </p>
                    <p className="pl-5 text-sm leading-relaxed text-gg-text2">{item.a}</p>
                  </div>
                ))}
              </div>
            </section>
            ...
            {/* ── CTA ──────────────────────────────────────────── */}
            <section>
              <div className="rounded-2xl border border-violet-500/20 bg-violet-600/5 p-10 text-center">
                <h2 className="text-2xl font-bold text-gg-text">Ready?</h2>
                <p className="mt-2 text-gg-text2">Open the dashboard and start managing your storage pool.</p>
                <div className="mt-6 flex flex-wrap justify-center gap-3">
                  <Link href="/login" className="rounded-xl bg-violet-600 px-6 py-3 font-semibold text-white shadow shadow-violet-600/25 transition hover:bg-violet-500">
                    Open Dashboard
                  </Link>
                  <Link href="/" className="rounded-xl border border-gg-border bg-gg-s1 px-6 py-3 font-semibold text-gg-text transition hover:border-violet-500/30">
                    Back to Home
                  </Link>
                </div>
              </div>
              <p className="mt-8 text-center text-xs text-gg-text3">
                GDriveGenie is free and open source.{" "}
                <a href="https://github.com/saimon4u/GDriveGenie" target="_blank" rel="noopener noreferrer" className="text-violet-400 hover:underline">View on GitHub.</a>
              </p>
            </section>
...
function StepHeader({ num, color, accent, title }: { num: string; color: string; accent: string; title: string }) {
  return (
    <div className={`mb-6 flex items-center gap-4 border-l-2 ${accent} pl-5`}>
      ...
      <h2 className="text-xl font-bold text-gg-text">{title}</h2>
    </div>
  );
}
