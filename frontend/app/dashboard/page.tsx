...
function StatCard({ label, value, sub, icon, iconBg }: StatCardProps) {
  return (
    <div className="rounded-xl border border-gg-border bg-gg-s1 p-5">
      ...
      <div className="text-2xl font-semibold text-gg-text">{value}</div>
      {sub && <div className="mt-0.5 text-xs text-gg-text3">{sub}</div>}
      <div className="mt-1 text-sm text-gg-text2">{label}</div>
    </div>
  );
}
  ...
  return (
    <div className="rounded-xl border border-gg-border bg-gg-s1 p-5">
      ...
      <div className="flex items-end justify-between gap-2">
        <div>
          <div className="text-2xl font-semibold text-gg-text">{used}</div>
          <div className="mt-0.5 text-xs text-gg-text3">used</div>
        </div>
        <div className="text-right">
          <div className="text-2xl font-semibold text-emerald-400">{free}</div>
          <div className="mt-0.5 text-xs text-gg-text3">free</div>
        </div>
      </div>
      <div className="mt-2 text-sm text-gg-text2">Storage</div>
    </div>
  );
}
...
function FileTypeIcon({ mimeType }: { mimeType: string | null }) {
  ...
  else if (t.includes("presentation") || t.includes("slide")) { bg = "bg-violet-600/10"; text = "text-violet-400"; label = "PPT"; }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="text-xl font-semibold text-gg-text">Overview</h1>
          <p className="mt-1 text-sm text-gg-text2">Your unified cloud storage at a glance</p>
        </div>
        {fromCache && (
          <Link
            href="/dashboard/stats"
            className="flex items-center gap-1.5 rounded-full border border-gg-border bg-gg-s1 px-3 py-1 text-xs text-gg-text2 transition hover:text-violet-400"
          >
      ...
      <div className="grid gap-4 lg:grid-cols-5">
        <div className="lg:col-span-3 rounded-xl border border-gg-border bg-gg-s1 p-5">
          <h2 className="mb-4 text-sm font-medium text-gg-text">Storage Breakdown</h2>
          <div className="mb-5">
            <div className="mb-1.5 flex items-end justify-between">
              <span className="text-lg font-semibold text-gg-text">{formatBytes(totalUsed)}</span>
              <span className="text-xs text-gg-text3">of {formatBytes(totalLimit)}</span>
            </div>
            <div className="h-2 w-full overflow-hidden rounded-full bg-gg-hover">
              <div
              ...
              return (
                <div key={a.account_index}>
                  <div className="mb-1 flex items-center justify-between text-xs">
                    <span className="max-w-[200px] truncate text-gg-text2">
                      {a.email ?? `Account ${a.account_index}`}
                    </span>
                    <span className="text-gg-text3">{formatBytes(a.used)} / {formatBytes(a.limit)}</span>
                  </div>
                  <div className="h-1.5 w-full overflow-hidden rounded-full bg-gg-hover">
                    <div className="h-full rounded-full bg-violet-600/60 transition-all" style={{ width: `${pct}%` }} />
                  </div>
                </div>
              );
            })}
            {connectedCount === 0 && <p className="text-sm text-gg-text3">No connected accounts</p>}
          </div>
        </div>

        <div className="lg:col-span-2 rounded-xl border border-gg-border bg-gg-s1 p-5">
          <h2 className="mb-4 text-sm font-medium text-gg-text">File Types</h2>
          <div className="space-y-2.5">
            {[
              ...
              { key: "Other", color: "bg-gg-text3", textColor: "text-gg-text2" },
            ]
              ...
              .map(({ key, color, textColor }) => (
                <div key={key} className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm text-gg-text2">{key}</span>
                  </div>
                  <span className={`text-sm font-medium ${textColor}`}>{s?.filesByType[key] ?? 0}</span>
                </div>
              ))}
            {!s && <p className="text-sm text-gg-text3">Loading…</p>}
            {s && Object.keys(s.filesByType).length === 0 && (
              <p className="text-sm text-gg-text3">No files yet</p>
            )}
          </div>
          <div className="mt-4 border-t border-gg-border pt-4">
            <Link href="/dashboard/stats" className="text-xs text-violet-400 hover:underline">
      ...
      {/* Recent files */}
      <div className="rounded-xl border border-gg-border bg-gg-s1">
        <div className="flex items-center justify-between border-b border-gg-border px-5 py-4">
          <h2 className="text-sm font-medium text-gg-text">Recent Files</h2>
          <Link href="/dashboard/files" className="text-xs text-gg-text2 transition hover:text-violet-400">
            View all →
          </Link>
        </div>
        {recentFiles.length === 0 ? (
          <div className="py-12 text-center">
            <p className="text-sm text-gg-text3">No files yet. Upload something to get started.</p>
          </div>
        ) : (
          <div className="divide-y divide-gg-border">
            {recentFiles.map((file) => (
              <div key={file.id} className="flex items-center gap-4 px-5 py-3 transition hover:bg-gg-hover">
                ...
                <div className="min-w-0 flex-1">
                  <p className="truncate text-sm text-gg-text">{file.file_name}</p>
                  <p className="text-xs text-gg-text3">
                    {formatBytes(file.size)} · Account #{file.account_index}
                  </p>
                </div>
                <span className="flex-shrink-0 text-xs text-gg-text3">
